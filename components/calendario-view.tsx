...
export default function CalendarioView() {
  ...
  const loadEventos = async () => {
    try {
      setLoading(true)
      const response = await fetch('/api/leads/eventos')
  ...
  }, [currentDate, dataSelecionada]) // Rerun if currentDate or dataSelecionada changes
    ...
    try {
      ...
      const response = await fetch('/api/leads/atividades/criar', {
        ...
        body: JSON.stringify({
          ...
          DATA_INICIO: new Date(novaAtividade.DATA_INICIO).toISOString(),
          DATA_FIM: new Date(novaAtividade.DATA_FIM || novaAtividade.DATA_INICIO).toISOString(),
